...
public enum MakerType implements BuildingType {
    IRON_MINE(null, 0, new ArrayList<>(List.of(Resources.IRON)) , 8, 5, 250,  new HashMap<>(Map.ofEntries(Map.entry(WOOD , 20))), 2),
    QUARRY(null, 0, new ArrayList<>(List.of(Resources.STONE)), 12, 20, 250,  new HashMap<>(Map.ofEntries(Map.entry(WOOD , 20))), 3),
    WOOD_CUTTER(null, 0, new ArrayList<>(List.of(WOOD)), 20 , 0,  100,  new HashMap<>(Map.ofEntries(Map.entry(WOOD , 3))), 1),
    ...
    BLACKSMITH(Resources.IRON, 4, new ArrayList<>(Arrays.asList(Weapons.SWORD , Weapons.MACE)), 1, 0,  250, new HashMap<>(Map.ofEntries(Map.entry(WOOD , 20) , Map.entry(GOLD , 100))), 1),
    POLETURNER(WOOD, 4, new ArrayList<>(Arrays.asList(Weapons.SPEAR , Weapons.PIKE)), 1, 0,  250, new HashMap<>(Map.ofEntries(Map.entry(WOOD , 10) , Map.entry(GOLD , 100))), 1),
    STABLE(null, 0, new ArrayList<>(List.of(Weapons.HORSE)), 1, 4, 250, new HashMap<>(Map.ofEntries(Map.entry(WOOD , 20) , Map.entry(GOLD , 400))), 0) ,
    APPLE_GARDEN(null, 0, new ArrayList<>(List.of(Food.APPLE)), 5, 0, 120, new HashMap<>(Map.ofEntries(Map.entry(WOOD , 5))), 1),
